// Name: NFCPlus
// ID: techguy16nfcplus
// Description: Allows reading data from NFC (NDEF) devices. 
// By: techguy16
// Context: NFC stands for "Near-field communication". Ideally check a real phone in your language to see how they translated it.

/* generated l10n code */Scratch.translate.setup({"it":{"_NFC supported?":"NFC supportato","_NFCWarp":"Sensore di Prossimità (NFC)","_Only works in Chrome on Android":"Funziona soltanto in Chrome per Android","_read NFC tag":"leggi tag NFC"},"nl":{"_NFC supported?":"NFC ondersteund?","_Only works in Chrome on Android":"Werkt alleen in Chrome op Android","_read NFC tag":"gegevens van NFC tag"},"zh-cn":{"_NFCWarp":"NFC","_read NFC tag":"读取NFC标签"}});/* end generated l10n code */(function (Scratch) {
  "use strict";

  /* globals NDEFReader */

  const extIcon =
    "data:image/png;base64,";
  const blocksIcon =
    "data:image/svg+xml;base64,";

  class NFCWarp {
    getInfo() {
      return {
        id: "techguy16nfcplus",
        name: Scratch.translate("NFCPlus"),
        color1: "#FF4646",
        color2: "#FF0000",
        color3: "#990033",
        menuIconURI: extIcon,
        blockIconURI: blocksIcon,
        blocks: [
          {
            blockType: Scratch.BlockType.LABEL,
            text: Scratch.translate("Only works in Chrome, Opera and Samsung Internet on Android"),
          },
          {
            opcode: "supported",
            blockType: Scratch.BlockType.BOOLEAN,
            text: Scratch.translate("NFC supported?"),
          },
          {
            opcode: "nfcRead",
            blockType: Scratch.BlockType.REPORTER,
            text: "read NFC [TYPE]",
            arguments: {
                TYPE: {
                    type: Scratch.ArgumentType.STRING,
                    menu: 'TYPE_MENU'
                }
            },
            disableMonitor: true,
          },
        ],
        menus: {
            TYPE_MENU: {
              acceptReporters: true,
              items: ['data', 'mac address']
            }
        }
      };
    }

    supported() {
      return typeof NDEFReader !== "undefined";
    }

    nfcRead(args) {
      if (!this.supported()) {
        return "NFC not supported";
      }
      return new Promise((resolve, reject) => {
        const ndef = new NDEFReader();
        ndef
          .scan()
          .then(() => {
            ndef.onreadingerror = (event) => {
              console.log("Reading error", event);
              resolve("Tag not supported");
            };
            ndef.onreading = (evt) => {
              const decoder = new TextDecoder();
              console.log(evt.serialNumber);
              const record = evt.message.records[0];
              console.log("Record type: " + record.recordType);
              console.log("Record encoding: " + record.encoding);
              if (args.TYPE == "data") {
                resolve(decoder.decode(record.data));
              } else if (args.TYPE == "mac address") {
                resolve(decoder.decode(evt.serialNumber));
              }
            };
          })
          .catch((error) => {
            console.log("Scan error", error);
            resolve(`Error: ${error}`);
          });
      });
    }
  }

  Scratch.extensions.register(new NFCWarp());
})(Scratch);
